...
# Each entry is "<assertion>|<node>:<count>", node "-" where the check is not per-node.
EXPECTED_ASSERTIONS=99
EXPECTED_ASSERT_MULTISET="H_before_settlement_deadline|-:1,H_margin_from_boundaries|-:1,H_not_epoch_boundary|-:1,app_height_after_halt_interval|0:1,app_height_after_halt_interval|1:1,app_height_after_halt_interval|2:1,app_height_after_halt_interval|3:1,binaries_differ|-:1,carry_unchanged_across_H|-:1,cleared_plan_response_canonical|-:1,clock_tick_at_H_plus_1|-:1,clock_tick_at_H|-:1,clock_total_across_two_blocks|-:1,cometbft_validators|-:1,coreslot_active_slots|-:1,economics_preserved_across_H|-:1,escrow_unchanged_across_H|-:1,halt_app_height|0:1,halt_app_height|1:1,halt_app_height|2:1,halt_app_height|3:1,halt_block_store_height|0:1,halt_block_store_height|1:1,halt_block_store_height|2:1,halt_block_store_height|3:1,halt_interval_measured|-:1,halt_logged_upgrade_required|0:1,halt_logged_upgrade_required|1:1,halt_logged_upgrade_required|2:1,halt_logged_upgrade_required|3:1,liability_unchanged_across_H|-:1,measured_clock_tick_per_block|-:1,migration_applied|0:1,migration_applied|1:1,migration_applied|2:1,migration_applied|3:1,migration_precondition_value|-:1,migration_ran_exactly_once|0:1,next_validators_hash_stable_across_H|-:1,pending_plan_cleared|0:1,pending_plan_cleared|1:1,pending_plan_cleared|2:1,pending_plan_height|0:1,pending_plan_height|1:1,pending_plan_height|2:1,pending_plan_height|3:1,pending_plan_name|0:1,pending_plan_name|1:1,pending_plan_name|2:1,pending_plan_name|3:1,restart_with_stale_metadata_resumed|0:1,running_binary_is_B|0:1,running_binary_is_B|1:1,running_binary_is_B|2:1,schedule_tx_delivered|-:1,settlement_deadline_clock_unchanged|-:1,settlement_entitlement_amount_unchanged|-:1,settlement_open_before_upgrade|-:1,settlement_released_amount_unchanged|-:1,settlement_remaining_amount_unchanged|-:1,solvency_after_boundary_pinned|-:1,solvency_before_boundary_pinned|-:1,solvency_before_boundary|-:1,stale_A_did_not_commit_H|3:1,stale_A_new_refusal_after_restart|3:1,stale_A_starts_from_H_minus_1|3:1,stale_node_caught_up_on_B|3:1,stale_node_launches_binary_A|3:1,stale_upgrade_info_height|0:1,stale_upgrade_info_name|0:1,stale_upgrade_info_present|0:1,stale_upgrade_info_still_present_after_restart|0:1,topology_valid|-:1,upgrade_info_height|0:1,upgrade_info_height|1:1,upgrade_info_height|2:1,upgrade_info_height|3:1,upgrade_info_name|0:1,upgrade_info_name|1:1,upgrade_info_name|2:1,upgrade_info_name|3:1,upgrade_info_valid|0:1,upgrade_info_valid|1:1,upgrade_info_valid|2:1,upgrade_info_valid|3:1,upgraded_majority_passed_H_plus_2|-:1,validator_hash_stable|-:1,validator_power_max|-:1,validator_power_min|-:1,validators_hash_stable_across_H|-:1,version_map_is_expected|0:1,version_map_is_expected|1:1,version_map_is_expected|2:1,version_map_is_expected|3:1,version_map_matches_node0|0:1,version_map_matches_node0|1:1,version_map_matches_node0|2:1,version_map_matches_node0|3:1,version_map_response_recorded|-:1"
...
finish() {
  ...
  if [[ -n "${EVID_DIR:-}" && -d "$EVID_DIR" ]]; then
    ...
    #
    # Keyed on the NODE as well as the name, because one assertion legitimately spans
    # two phases: migration_applied covers nodes 0-2 when the upgraded majority
    # crosses the boundary, and node 3 after the stale node rejoins — two different
    # claims. Keyed on the name alone, a count of four is satisfied by any split, so a
    # lost node-3 check paired with a double-counted node would balance out. Keyed on
    # the node it does not: every one of the 99 keys is expected exactly once.
    observed="$(jq -r '"\(.assertion)|\(.node)"' "$UPGRADE_LOG" 2>/dev/null | sort | uniq -c \
