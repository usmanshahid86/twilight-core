...
each epoch's reward pool by active-block participation, and holds the result as a
per-slot entitlement until settlement releases it. It depends on CoreSlot for the
active operator set but **never** manages validators itself.
...
2. **EndBlock** — when the configured epoch boundary is reached, the module
   ...
   active blocks, writes an immutable epoch aggregate plus one **slot
   entitlement** per eligible slot, advances the carry-forward remainder and
   cumulative emitted, and opens the next epoch.
3. **Settlement** — value leaves escrow only through `x/mining`, which pays the
   epoch's participants and returns the remainder to the slot's **snapshotted
   payout address**. Rewards exposes this as a constrained keeper boundary with
...
- **Module accounts** — `{constants.rewardsModuleAccount}` (the `Minter`,
  holding minted emission and unreleased entitlements) and
...
- **Bank** — mints `{constants.nativeDenom}` into the rewards account and sends
  released value from it.

## Authority model

- **Params updates** are authorized by the CoreSlot **authority** and queued to
  activate at the next epoch boundary.
- **Pause / resume** is authorized by the CoreSlot **emergency authority** and
  takes effect at the beginning of the next block. There is one canonical pause
  state, not per-area selectors.
