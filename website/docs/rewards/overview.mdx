...
`x/rewards` mints scheduled `{constants.nativeDenom}` block rewards and allocates
each epoch's reward pool to the active CoreSlot operators, then lets those rewards
...
- **CoreSlot** (`x/coreslot`) — provides the active operator set, each slot's
  operator/payout addresses, and reward weights. Rewards reads these through a
